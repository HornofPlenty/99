...
contract BNBSeed {
	...
	constructor(address payable wallet) public {
        ...
        plans.push(Plan(10000, 259200));
		plans.push(Plan(150, 403200));
		plans.push(Plan(80, 547200));
        plans.push(Plan(21, 14400));
        plans.push(Plan(7, 14400));
	}
	...
	function invest(address referrer, uint8 plan) public payable {
		...
		for (uint256 i = 0; i < users[msg.sender].deposits.length; i++) {
			if(user.deposits[i].plan < 3){
				if(user.deposits[i].start.add(plans[user.deposits[i].plan].time.mul(1 days)) > block.timestamp){
		...
		for (uint256 i = 0; i < user.deposits.length; i++) {
			if(user.deposits[i].start.add(plans[user.deposits[i].plan].time.mul(1 days)) > block.timestamp){
				if(user.deposits[i].plan==3){
					...
					daliybonus = daliybonus.add(user.deposits[i].amount);
				}else if(user.deposits[i].plan==5){
					daliybonus = daliybonus.add(user.deposits[i].amount);
				}else if(user.deposits[i].plan==6){
					daliybonus = daliybonus.add(user.deposits[i].amount);
				}else if(user.deposits[i].plan==7){
					daliybonus = daliybonus.add(user.deposits[i].amount);
				}else if(user.deposits[i].plan==8){
					daliybonus = daliybonus.add(user.deposits[i].amount);
				}
			}
		}
	...
	function getUserInfo(address userAddress) public view returns(uint256 totalDeposit, uint256 totalWithdrawn, uint256 totalReferrals, uint256 yourboomplan, uint256 yourbonuspercent, uint256 Userbonuspercent) {
		return(getUserTotalDeposits(userAddress), getUserTotalWithdrawn(userAddress), getUserTotalReferrals(userAddress), getyourboomplan(userAddress), getyourbonuspercent(userAddress), getUserbonuspercent(userAddress));
	}
